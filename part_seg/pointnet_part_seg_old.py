...
def get_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    with tf.variable_scope('transform_net2') as sc:
        K = 64
        transform = get_transform_K(out2, is_training, bn_decay, K)

    end_points['transform'] = transform

    squeezed_out3 = tf.reshape(out2, [batch_size, num_point, 64])
    net_transformed = tf.matmul(squeezed_out3, transform)
    net_transformed = tf.expand_dims(net_transformed, [2])

    out4 = tf_util.conv2d(net_transformed, 64, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv4', bn_decay=bn_decay)
    out5 = tf_util.conv2d(out4, 128, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv5', bn_decay=bn_decay)
    out6 = tf_util.conv2d(out5, 1024, [1, 1], padding='VALID', stride=[1, 1],
                          bn=True, is_training=is_training, scope='conv6', bn_decay=bn_decay)
    out_max = tf_util.max_pool2d(out6, [num_point,1], padding='VALID', scope='maxpool')
    ...
    net2 = tf_util.conv2d(concat, 512, [1,1], padding='VALID', stride=[1,1], bn_decay=bn_decay,
    ...
    net2 = tf_util.conv2d(net2, 128, [1, 1], padding='VALID', stride=[1, 1], bn_decay=bn_decay,
                          bn=True, is_training=is_training, scope='seg/conv4', weight_decay=weight_decay)
